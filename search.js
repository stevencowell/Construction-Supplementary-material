(function() {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      /* Ensure home page search is always visible above header elements */
  ...
  function ensureSearchUI() {
    ...
    // On home page, place search after the overview section to avoid header overlap
    let containerMountTarget;
    if (home) {
      const overviewSection = main ? main.querySelector('.overview') : null;
      containerMountTarget = overviewSection ? overviewSection.nextSibling : (main || document.body);
    } else {
      containerMountTarget = (header || main || document.body);
    }
    ...
    if (!container) {
      ...
      // Place the container in the appropriate target
      if (home && containerMountTarget === main) {
        // On home page, insert after overview section
        const overviewSection = main.querySelector('.overview');
        if (overviewSection && overviewSection.nextSibling) {
          main.insertBefore(container, overviewSection.nextSibling);
        } else {
          main.appendChild(container);
        }
      } else {
        // For non-home pages or fallback
    ...
    // Ensure search container is properly positioned on home page
    if (home && container.parentElement === main) {
      const overviewSection = main.querySelector('.overview');
      if (overviewSection && container.previousSibling !== overviewSection) {
        // Move search container to be right after overview section
        main.insertBefore(container, overviewSection.nextSibling);
      }
    }
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', function() {
      ...
      // Ensure proper positioning on home page after a brief delay
      if (isHomePage()) {
        setTimeout(() => {
          ensureSearchUI();
        }, 100);
